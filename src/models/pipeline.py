...
from configs.settings import get_settings, MODELS_DIR
...
class ALPRPipeline:
    ...
    def _load_yolo_vehicle(self):
        ...
        try:
            if model_path:
                ...
                # Télécharger yolov8s.pt automatiquement
                self.yolo_vehicle = YOLO("yolov8s.pt")
                print("✅ YOLO Véhicule: yolov8s.pt (téléchargé)")
                
        except Exception as e:
            print(f"⚠️ YOLO Véhicule non disponible: {e}")
    ...
    def _load_brand_classifier(self):
        ...
        try:
            ...
            # Déterminer le nombre de classes depuis le state_dict
            # La dernière couche FC a shape (num_classes, features)
            num_classes = len(self.model_config.brand_classes)
            
            # Essayer de détecter depuis le state_dict
            ...
            if TIMM_AVAILABLE:
                ...
                # Fallback: créer manuellement avec torchvision
                from torchvision.models import efficientnet_b4
                self.brand_model = efficientnet_b4(weights=None)
                self.brand_model.classifier[1] = nn.Linear(
    ...
    def read_plate(self, plate_roi: np.ndarray) -> Tuple[str, float]:
        """
        Lit les caractères d'une plaque.
        
        Args:
            plate_roi: Image de la plaque (BGR ou RGB)
        ...
        try:
            ...
            if ocr_result and len(ocr_result) == 2:
                ...
                if plates and len(plates) > 0:
                    text = plates[0]
                    ocr_conf = float(np.mean(confidences[0])) if len(confidences) > 0 else 0.8
                    return text.upper(), ocr_conf
