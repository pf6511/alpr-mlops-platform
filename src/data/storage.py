...
class StorageManager:
    ...
    def __init__(self):
        ...
        # Déterminer le mode
        if self.s3_config.is_s3 and S3_AVAILABLE:
            self.mode = "s3"
        ...
        print(f"📦 Storage: {self.mode.upper()}" + 
              (f" → {self.s3_config.endpoint_url}" if self.mode == "s3" else " → Local"))
    ...
    def _init_local_storage(self):
        ...
        self.local_paths = {
            'dataset': Path(self.s3_config.local_dataset_path),
            'captures': Path(self.s3_config.local_captures_path),
            'artifacts': Path(self.s3_config.local_dataset_path).parent / "artifacts"
        }
    ...
    def _ensure_buckets_exist(self):
        ...
        buckets = [
            self.s3_config.dataset_bucket,
            self.s3_config.captures_bucket,
    ...
    def _get_bucket(self, bucket_type: str) -> str:
        ...
        buckets = {
            'dataset': self.s3_config.dataset_bucket,
            'captures': self.s3_config.captures_bucket,
            'artifacts': self.s3_config.artifacts_bucket
        }
        return buckets.get(bucket_type, self.s3_config.dataset_bucket)
    ...
    def _upload_s3(self, file_path: Path, key: str, bucket_type: str, 
        ...
        try:
            ...
            url = f"{self.s3_config.endpoint_url}/{bucket}/{key}"
            return True, url
            ...
            try:
                ...
                for obj in response.get('Contents', []):
                    files.append(StorageFile(
                        ...
                        bucket=bucket,
                        url=f"{self.s3_config.endpoint_url}/{bucket}/{obj['Key']}"
                    ))
                return files
